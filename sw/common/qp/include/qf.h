...
* \include qf_start.c
*/
void QActive_start(QActive * const me, uint8_t prio, QEvt const *qSto[], uint32_t qLen, void *stkSto, uint32_t stkSize, QEvt const *ie);
...
* uses this function to recall a deferred event from a given QF
* event queue. Recalling an event means that it is removed from the
* deferred event queue \a eq and posted (FIFO) to the event queue of
* the active object.
*
* QActive_recall() returns 1 (TRUE) if an event has been recalled.
* Otherwise the function returns 0.
...
* different kinds.
*
* \sa QActive_recall(), QEQueue, QActive_postFIFO()
*/

uint8_t QActive_recallFIFO(QActive * const me, QEQueue * const eq);

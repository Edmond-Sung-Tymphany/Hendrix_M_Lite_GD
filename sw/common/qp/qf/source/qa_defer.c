...
uint8_t QActive_recallFIFO(QActive * const me, QEQueue * const eq) {
    ...
    if (e != (QEvt const *)0) {                         /* event available? */
        ...
        /*Note, when implement QActive_defer(), an event will be posted
        * to an FIFO queue, when post multi events, the recall event sequence should be careful
        */
        QActive_postFIFO(me, e);

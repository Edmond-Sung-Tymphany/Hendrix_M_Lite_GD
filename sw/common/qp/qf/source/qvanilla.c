...
int16_t QF_run(void) {
    ...
    for (;;) {                                       /* the background loop */
        ...
        if (QPSet64_notEmpty(&QF_readySet_)) {
            ...
            /* Tymphany code start, for stack overflow check*/
#ifdef HAS_STACK_OVERFLOW_LOW_LEVEL_CHECK
            #define SNPRINTF_BUF_MAX_SIZE                (200)
            if(stackOverflowCheck())
            {
                char buffer[SNPRINTF_BUF_MAX_SIZE];
                QHsm* tmp = &a->super;
                int funcPtr = (int)(tmp->state.fun);
                if((void *)0 != tmp)
                {
                    snprintf(buffer,SNPRINTF_BUF_MAX_SIZE,"Stack overflow with function address: 0x%08x,\r\n%s",funcPtr,__FILE__);
                }
                else
                {
                    snprintf(buffer,SNPRINTF_BUF_MAX_SIZE,"Stack overflow\r\n%s",__FILE__);
                }
                printf(buffer);
                BSP_ExpBlockingDelayMs(100); //wait print complete
                Q_onAssert(buffer,__LINE__);
            }
#endif
            /* Tymphany code end */
...
void QActive_stop(QActive * const me) {
    /* Fix issue: [L]DUT won't factory reset and will hang after press and hold & button for 7 secs. [IN:013474] 
    * https://pm.tymphany.com/SpiraTeam/289/Incident/13474.aspx
    */

...
static AJ_Status ReadLine(AJ_IOBuffer* rxBuf) {
    ...
    /* Tymphany Modified.
     * Changed by Tym to shorten blocking time when powering up.
     */
    AJ_Time timer;
    AJ_InitTimer(&timer);
    while (((AJ_IO_BUF_AVAIL(rxBuf) == 0) || (*(rxBuf->writePtr - 1) != '\n')) && AJ_GetElapsedTime(&timer, TRUE) < 3500)
    {
        status = rxBuf->recv(rxBuf, AJ_IO_BUF_SPACE(rxBuf), 100);
    }
...
AJ_Status AJ_Connect(AJ_BusAttachment* bus, const char* serviceName, uint32_t timeout)
{
    ...
    /* Tymphany modified.*/
#if !(defined(ARDUINO) || defined(__linux) || defined(_WIN32) || defined(__MACH__) || defined(PIC32) /*PIC32 TARGET added by TYM*/ )
    /*

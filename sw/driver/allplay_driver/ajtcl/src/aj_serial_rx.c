...
void AJ_ReceiveCallback(uint8_t* buffer, uint16_t bytesRead)
{
    ...
    /* Tymphany modified.*/
    /* When upgrade fail, AudioSrv is busy on initializing DSP and cause AllplaySrv can not handle Allplay Data,
     * finally buffer is overflow (NULL==pendingRecvBuffer).
     * Thus We ignore data for this special case.
     */
    if(NULL==pendingRecvBuffer)
    {
        AJ_AlwaysPrintf(("\r\n*** AJ_ReceiveCallback WARNING: Ignore %u bytes data due to buffer overflow\r\n\r\n", bytesRead));
        return;
    }
    ...
    /* Tymphany modified.*/
    /* Chekc if we have free buffer to store data */
    assert(pendingRecvBuffer);
...
static void CompletePacket()
{
    ...
    /* Tymphany modified.*/
    //uint8_t checksum;  //remove unuse variable

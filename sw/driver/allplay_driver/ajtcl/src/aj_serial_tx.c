...
#include "aj_debug.h"
/* Tymphany modified.*/
#include "aj_config.h"  // AJ_TX_SERIAL_SEND_TIMEOUT_MS
...
/* Tymphany modified. */
extern void AJ_TX(uint8_t* buf, uint32_t len);
...
void AJ_SerialTX_Shutdown(void)
{
    ...
    /* Tymphany modified.*/
#ifdef QUALCOMM_ALLPLAY_MCULIB_ORIGINAL
    if (txUnreliable == txQueue) {
#else
    // Wesley: bug fix for shutdown at SAM powering-up
    if ((txUnreliable == txQueue) && (txUnreliable != NULL)){
#endif
                        ...
                        uint16_t bufLen)
{
    ...
    while (len) {
        ...
        if (!txFreeList || AJ_SerialLinkParams.linkState != AJ_LINK_ACTIVE) {
              ...
              *   For MCU v4.05 / SAM v1.5.29-s, when SAM reboot (MCU does not reboot),
              *   MCU may stay in infinite loop here (because txFreeList==NULL), and cause watchdog timeout.
              *   I wonder the fininite loop is due to AJ_SerialTx_ReceivedAck() does not get ACK, then txFreeList is always NULL 
              * [Solution]
              *   Add timeout here
              */
            AJ_InitTimer(&now);
            if( AJ_GetTimeDifference(&now, &start) >= AJ_TX_SERIAL_SEND_TIMEOUT_MS )  {
                AJ_AlwaysPrintf(("\r\n*** AJ_SerialSend WARNING: timeout (%dms) to wait ACK. linkState=%d ***\r\n\r\n", AJ_TX_SERIAL_SEND_TIMEOUT_MS, AJ_SerialLinkParams.linkState ));
                status = AJ_ERR_LINK_DEAD;
                break;
            }
            else {
        ...
        while (txFreeList && len) {
            ...
            /* Tymphany modified.*/
            /* Re-initialize timer after send some data */
            AJ_InitTimer(&start);
        }
    }
    return status;

...
/*****************************************************************************/
{
    ...
    //Tymphany: look like it is a redundency timer setting, will replace later
...
//            }
    ...
    //ASSERT(pTouchKeyDrv && pTouchKeyDrv->super_.isCreated);
...
/*****************************************************************************/
{
    ...
    {
        /* Azoteq Johaton 2016/1/18: 
         *   whick improves
         *   Then the second adition is in I2C_Comms.c add a delay just before the STOP condition
         * Tymphany:
         *   What I guess for this workaround is, IQS333 sometimes close window when sense any STOP.
         *   But if other STOP is sent soon after last data, IQS333 may not close windows.
         *   This workaround ensure IQS333 close windows (it will sense a STOP after 50us delay),
         *   then  state_machine_process() is 100% sure IQS333 is closed, then set iqs333_sm->RDY_Window = 0;
         */
        if (iqs->I2C_Address == IQS5XX_ADDR)
        {
            Delay_us(50);
        }
...
/*****************************************************************************/
{
    ...
    /* Tymphany:
     *   The follwoing code is modified from sample code, it seems be generic for 
     *   Azoteq series products, but very complex for IQS572 / IQS333.
     *   The rule is very simple: only access I2C when RDY is ready 
     */
      
#if 0    
    uint32 now_ms= getSysTime();
    uint32 timeout_ms= now_ms + IQS_WAIT_RDY_TIMEOUT_MS;

    if ((iqs->Type == RDY_Active_High) && (iqs->Poll != Polling)) //I2C wait for READY (active high)
    {
        //Tymphany: IQS333 is ActiveLow, and IQS572 is Poll, thus we never go to here
        ASSERT(0);
    
        //if we are in event mode, check whether we first need to wake the device before sending command
        if (iqs->Event_Mode == Active)
        {
            if(IQS_Get_RDY_Status(iqs))
            {
                /* Do Event mode handshake - new way */
        ...
        while( !IQS_Get_RDY_Status(iqs) && (now_ms<timeout_ms) ) {
            now_ms= getSysTime();
        }
        //while(!GPIO_ReadInputDataBit(iqs->RDY_Port, iqs->RDY_Pin) && !Timer4Expired); //Original sample code
    }

    if ((iqs->Type == RDY_Active_Low) && (iqs->Poll != Polling)) //I2C wait for READY (active low)
    {
        //Tymphany: Only IQS333 go to here, wait RDY_333==0
        
        //if we are in event mode, check whether we first need to wake the device before sending command
        if (iqs->Event_Mode == Active)
        {
            //Tymphany: becuase IQS333 is Stream mode (Inactive), and IQS572 is Poll, thus we never go to here
            ASSERT(0);
          
            if(IQS_Get_RDY_Status(iqs))
            {
                /* Do Event mode handshake - new way */
                I2C_Event_Mode_Handshake(iqs); //original sample
            }
        }
...
/* Tymphany Add */
void I2C_Set_StopOperation(I2C_Device_t* iqs, I2C_Stop_t stop)
{
    iqs->Stop= stop;
    if(stop==I2C_Stop) {
        I2CDrv_SetStopOperation(iqs->pI2cDrv, /*stopEnable:*/TRUE);
    }
    else if(stop==I2C_Repeat_Start) {
        I2CDrv_SetStopOperation(iqs->pI2cDrv, /*stopEnable:*/FALSE);
    }
    else {
        ASSERT(0);
    }
 ...
 * @retval    [uint8_t] Device Ready - 0 is not ready and >0 is ready
 */

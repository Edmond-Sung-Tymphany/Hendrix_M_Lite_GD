...
uint8_t IQS_Setup_Devices(void)
{
    ...
    /* Tymphany add */
    iqs360a_cmds->pI2cDrv = &pTouchKeyDrv->i2c360aDrv;
    iqs5xx_cmds->pBlI2cDrv = NULL;
    iqs360a_cmds->RDY_GpioPin = GPIO_IN_TCH_360_RDY;
    iqs360a_cmds->settingId = SETID_IQS360A_CONNECTED;
    
    ASSERT(IQS360A_ADDR==(iqs360a_cmds->pI2cDrv->pConfig->devAddress>>1));
    ...
    /* Tymphany add */
    iqs5xx_cmds->pI2cDrv = &pTouchKeyDrv->i2c572Drv;
    iqs5xx_cmds->pBlI2cDrv = &pTouchKeyDrv->i2c572BlDrv;
    iqs5xx_cmds->RDY_GpioPin = GPIO_IN_TCH_572_RDY;
    iqs5xx_cmds->settingId = SETID_IQS572_CONNECTED;
...
void IQS_RDY_window(I2C_Device_t* iqs)
{
	...
	{
		/* Device is RDY Active High */
		if (iqs->Type == RDY_Active_High)
		{
			iqs->RDY_Window = 1;	// open window
//#ifdef DEBUG
//			GPIO_SetBits(DEBUG_PORT, DEBUG_IO);
//			Delay_us(2);
//			GPIO_ResetBits(DEBUG_PORT, DEBUG_IO);
//#endif
		}
		/* Device is RDY Active Low */
		else if(iqs->Type == RDY_Active_Low)
		{

...
static void PwmLedDrv_pinConfig(const tPwmLedMap* pPwmLedMap)
{
    ...
    uint16 GPIO_Pin_x;
    uint8 GPIO_PinSourceX;
    switch(pPwmLedMap->pwmPin.gpioPort)
    {
        case IO_PORT_A:
            RCC_AHBPeriph_x = RCC_AHBPeriph_GPIOA;
            GPIOX = GPIOA;
            break;
        case IO_PORT_B:
            RCC_AHBPeriph_x = RCC_AHBPeriph_GPIOB;
            GPIOX = GPIOB;
            break;
        case IO_PORT_C:
            RCC_AHBPeriph_x = RCC_AHBPeriph_GPIOC;
            GPIOX = GPIOC;
            break;
        default:
            ASSERT(0);
    ...
    RCC_AHBPeriphClockCmd( RCC_AHBPeriph_x, ENABLE);
    GPIO_Pin_x = GPIO_Pin_0<<(pPwmLedMap->pwmPin.gpioBit);
    GPIO_PinSourceX = (pPwmLedMap->pwmPin.gpioBit);
    ...
    GPIO_PinAFConfig(GPIOX, GPIO_PinSourceX, GPIO_AF_2);

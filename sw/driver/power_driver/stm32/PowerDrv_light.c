...
static tpowerUpSequence awakeningSequence[] =
{
    {&PowerDrv_AwakeStage1, GET_TICKS_IN_MS(POWER_STAGE1_DELAY)},
    {&PowerDrv_AwakeStage2, GET_TICKS_IN_MS(POWER_STAGE2_DELAY)},
    {&PowerDrv_AwakeStage3, GET_TICKS_IN_MS(POWER_STAGE3_DELAY)},
};
#endif
...
uint16  PowerDrv_AwakePower(cPowerDrv *me)
{
    /*need to change uint16 to int32, but keep it for now as it's used in other project*/
    uint16 delayTime;
    if(me->step < Q_DIM(awakeningSequence))
    {
        /* call the power up function according to step*/
        awakeningSequence[me->step].powerUpFunction(me);
        /* get delay time*/
        delayTime = awakeningSequence[me->step].delaytime;
        me->step ++;
...
{
    bool ret = FALSE;
    int16 hwVersionRawVoltage = 0;

    if (adcDrv.isCreated)
    {
        hwVersionRawVoltage = (int16)ADCDrv_GetData(&adcDrv, adcDrv.ADCConfig->pAdcPinIoAttr[0].adcPin);

        if(ADC_DATA_IS_NOT_READY != hwVersionRawVoltage)
        {
            *rawVoltage = (hwVersionRawVoltage * ADC_REFERENCE_mVOLT) / ADC_PRECISION;
            ret = TRUE;
        }
    }
    return ret;

...
void MainApp_SendLedReq2(cMainApp* me, eLedIndID ledIndID,
            tGroupPatternData *pattern_tbl, uint8 volume, uint8 *volToOneLed_tbl)
{
    ASSERT(ledIndID < LED_IND_ID_MAX);
    //TP_PRINTF("MainApp_SendLedReq2: ledIndID=%d\r\n", ledIndID);
    LedSrvGroup_SetPatt((QActive *)me, me->ledInds[ledIndID].leds, me->ledInds[ledIndID].patternId,
                pattern_tbl, volume, volToOneLed_tbl);
}
...
void MainApp_Mute(cMainApp* me, bool muteEnable2)
{
    ...
    if(me->audioSource!=AUDIO_SOURCE_COAXIAL)
    {

...
int main(int argc, char** argv)
{
    ...
    stackMagicNumberFill();                 //Fill magic numbers in the bottom side of stack space
    ...
    for(i = 0; i < 48; i++)
    {
        VectorTable[i] = *(__IO uint32_t*)(MEMORY_ADDR_APP + (i<<2));
    }
    ...
    // reset here for better powering up procedure
#ifdef HAS_IOE_LED
    IoeLed_Ctor();
    IoeLed_Reset();
#endif
...
#ifdef HAS_IWDG
    IwdgInit(IWDG_Prescaler_16, 0x0fff);    // around 1.6 second
#endif

...
{
    ASSERT(pAseFepEvent);
    Proto_Dsp_ToneTouch *pTTouch= &pAseFepEvent->data.dspToneTouch;
      
    //TP_PRINTF("MainApp_AseNgToneTouchHandler: which_data=%d, has_vol=%d, vol=%d, %dms\r\n", pAseFepEvent->which_data, pVolEvt->has_volume, pVolEvt->volume, pVolEvt->fade_duration);
    if( pAseFepEvent->which_data==Proto_AseFep_Event_dspToneTouch_tag )
    {
        Proto_Dsp_ToneTouch *pToneTouch= &(pAseFepEvent->data.dspToneTouch);
        if( pToneTouch->has_enabled ) {
            Setting_Set(SETID_TONE_TOUCH_ENABLED, &(pToneTouch->enabled));
        }
        
        //All ToneTouch value have valid range between -1.0 ~ 1.0
        if( pToneTouch->has_Gx1 ) {
            if( pToneTouch->Gx1 >=-1.0 && pToneTouch->Gx1<=1.0 ) {
                Setting_Set(SETID_TONE_TOUCH_GX1, &(pToneTouch->Gx1));
            }
            else  {
                TP_PRINTF("\r\n*** ASE set invalid ToneTouch value: Gx1=%f\r\n\r\n", pToneTouch->Gx1 );
            }
        }
        if( pToneTouch->has_Gx2 ) {
            if( pToneTouch->Gx2 >=-1.0 && pToneTouch->Gx2<=1.0 ) {
                Setting_Set(SETID_TONE_TOUCH_GX2, &(pToneTouch->Gx2));
            }
            else {
                TP_PRINTF("\r\n*** ASE set invalid ToneTouch value: Gx2=%f\r\n\r\n", pToneTouch->Gx2 );
            }
        }
        if( pToneTouch->has_Gy1 ) {
            if( pToneTouch->Gy1 >=-1.0 && pToneTouch->Gy1<=1.0 ) {
                Setting_Set(SETID_TONE_TOUCH_GY1, &(pToneTouch->Gy1));
            }
            else {
                TP_PRINTF("\r\n*** ASE set invalid ToneTouch value: Gy1=%f\r\n\r\n", pToneTouch->Gy1 );
            }
        }
        if( pToneTouch->has_Gy2 ) {
            if( pToneTouch->Gy2 >=-1.0 && pToneTouch->Gy2<=1.0 ) {
                Setting_Set(SETID_TONE_TOUCH_GY2, &(pToneTouch->Gy2));
            }
            else {
                TP_PRINTF("\r\n*** ASE set invalid ToneTouch value: Gy2=%f\r\n\r\n", pToneTouch->Gy2 );
            }
        }
        if( pToneTouch->has_Gz ) {
            if( pToneTouch->Gz >=-1.0 && pToneTouch->Gz<=1.0 ) {
                Setting_Set(SETID_TONE_TOUCH_GZ, &(pToneTouch->Gz));
            }
            else {
                TP_PRINTF("\r\n*** ASE set invalid ToneTouch value: Gz=%f\r\n\r\n", pToneTouch->Gz );
            }
        }
        if( pToneTouch->has_k5 ) {
            if( pToneTouch->k5 >=-1.0 && pToneTouch->k5<=1.0 ) {
                Setting_Set(SETID_TONE_TOUCH_K5, &(pToneTouch->k5));
            }
            else {
                TP_PRINTF("\r\n*** ASE set invalid ToneTouch value: K5=%f\r\n\r\n", pToneTouch->k5 );
            }
        }
        if( pToneTouch->has_k6 ) {
            if( pToneTouch->k6 >=-1.0 && pToneTouch->k6<=1.0 ) {
                Setting_Set(SETID_TONE_TOUCH_K6, &(pToneTouch->k6));
            }
            else {
                TP_PRINTF("\r\n*** ASE set invalid ToneTouch value: K6=%f\r\n\r\n", pToneTouch->k6 );
            }
        }
...
//        pAudioEvt->has_local)
//    {
//        //ASENG_TP_PRINTF("ASE-NG Set Local playback: %d\r\n", pAudioEvt->local);
//        if( pAudioEvt->local )
//        {
...
//    }
        if( pAseFepReq->which_data == Proto_AseFep_Req_audioInput_tag )
    {
            if( pAudioEvt->has_input)
        {
                TP_PRINTF("ASE-NG Set Input: %d\r\n", pAudioEvt->input);
                switch (pAudioEvt->input)
                {
                case Proto_Dsp_RequestAudioInput_AudioInput_ASE:
        {
            MainApp_SwitchAudioSource(me, AUDIO_SOURCE_ASE);
//                    uint8 source = AUDIO_SOURCE_ASETK;
//                    Setting_Set(SETID_CHANNEL, &source);
                    break;
                }
                case Proto_Dsp_RequestAudioInput_AudioInput_TOS_LINK:
                {
                    MainApp_SwitchAudioSource(me, AUDIO_SOURCE_SPDIF_IN);
//                    uint8 source = AUDIO_SOURCE_EXT_SOURCE;
//                    Setting_Set(SETID_CHANNEL, &source);
                    break;
                }
                case Proto_Dsp_RequestAudioInput_AudioInput_LINE:
                {
                    MainApp_SwitchAudioSource(me, AUDIO_CHANNEL_AUXIN);
//                    uint8 source = AUDIO_SOURCE_EXT_SOURCE;
//                    Setting_Set(SETID_CHANNEL, &source);
//                    MainApp_SendLedReq(me, LED_IND_ID_BT_PAIRING_FAIL, /*force:*/TRUE);
                    break;
                }
                default:
                    break;

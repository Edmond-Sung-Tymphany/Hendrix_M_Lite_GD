...
void MainApp_DelayedErrorReboot(cMainApp * const me)
{
    ...
    /* When DSP is unstable and not define HAS_DSP_UNSTABLE_REBOOT, 
    /* => wait few seconds (for finished LED indication), then reboot system
     */
...
eLedIndID MainApp_GetUpdatedProdLed(cMainApp * const me)
{
     ...
     * if not define HAS_DSP_UNSTABLE_REBOOT:
     *   For normal mode, flash RED led to indicate this error
     *   For shop mode, not change LED
     */
#ifndef HAS_DSP_UNSTABLE_REBOOT
    isDspErr= *(bool*)Setting_GetEx(SETID_IS_DSP_ERROR, &isDspErr);
#endif
    
    TP_PRINTF("\r\nMainApp_GetUpdatedProdLed: sysTempLevel=%d, systemStatus=%d, isDspErr=%d *** \r\n",
                 me->sysTempLevel,
                 me->systemStatus,
                 isDspErr);

    if( me->sysTempLevel==TL_CRITICAL || isDspErr )
    {
...
//                AseTkSrv_SendFepAseEvent(FepAseEvent_Event_LINE_SENSE_ACTIVE);
                  TP_PRINTF("\r\nMainApp_MusicDetectSignalHandler: Proto_FepAse_Event_Type_LINE_IN_SENSE_ACTIVE...");
                  AseNgSrv_SendFepAseEvent(Proto_FepAse_Event_Type_LINE_IN_SENSE_ACTIVE);
                  //DebugSSrv_PrintStr("Proto_FepAse_Event_Type_LINE_IN_SENSE_ACTIVE");
            }
            else
            {
                  TP_PRINTF("\r\nMainApp_MusicDetectSignalHandler: Proto_FepAse_Event_Type_LINE_IN_SENSE_INACTIVE...");
                  AseNgSrv_SendFepAseEvent(Proto_FepAse_Event_Type_LINE_IN_SENSE_INACTIVE);
                  //DebugSSrv_PrintStr("Proto_FepAse_Event_Type_LINE_IN_SENSE_INACTIVE");
            }
            break;
      }
    case SPDIF1_IN_JACK:
      {
            if(musicOn)
            {
                  TP_PRINTF("\r\nMainApp_MusicDetectSignalHandler: Proto_FepAse_Event_Type_TOSLINK_SENSE_ACTIVE...");
//                AseTkSrv_SendFepAseEvent(FepAseEvent_Event_TOSLINK_SENSE_ACTIVE);
                    AseNgSrv_SendFepAseEvent(Proto_FepAse_Event_Type_TOSLINK_SENSE_ACTIVE);
                  //DebugSSrv_PrintStr("Proto_FepAse_Event_Type_TOSLINK_SENSE_ACTIVE");
            }
            else
            {
                  TP_PRINTF("\r\nMainApp_MusicDetectSignalHandler: Proto_FepAse_Event_Type_TOSLINK_SENSE_INACTIVE...");
                    AseNgSrv_SendFepAseEvent(Proto_FepAse_Event_Type_TOSLINK_SENSE_INACTIVE);
                    //DebugSSrv_PrintStr("Proto_FepAse_Event_Type_TOSLINK_SENSE_INACTIVE");
            }
            break;
      }
    case JACK_IN_INVALID: // means no jack plugged - ASE as audio source
      {
            if(musicOn)
            {
...
//                AseTkSrv_SendFepAseEvent(FepAseEvent_Event_TOSLINK_SENSE_ACTIVE);
            ...
            {
//                AseTkSrv_SendFepAseEvent(FepAseEvent_Event_TOSLINK_SENSE_INACTIVE);
            }
            break;
      }

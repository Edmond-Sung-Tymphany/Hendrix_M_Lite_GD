#ifndef _PATTERN_H
#define _PATTERN_H

#include "attachedDevices.h"


/* the pattern ID which is defined in UI*/
typedef enum
{
    OFF_PATT = 0,

    /* solid pattern */
    SOLID_PAT_WHITE,
    SOLID_PAT_RED,
    SOLID_PAT_GREEN,
    SOLID_PAT_BLUE,
    SOLID_PAT_AMBER,
    SOLID_PAT_YELLOW,

    /* dim pattern */
    DIM_PATT_WHITE,
    DIM_PATT_AMBER,
    DIM_PATT_RED,
    DIM_PATT_BLUE,

    /* flash pattern */
    FLASH_PAT_AMBER,
    FLASH_PAT_BLUE,
    
    /* quick flash pattern */
    QUICK_FLASH_PAT_RED,
    QUICK_FLASH_PAT_WHITE,
    QUICK_FLASH_PAT_ORANGE,
    QUICK_FLASH_PAT_YOLLOW,
    QUICK_FLASH_ONCE_PAT_WHITE,

    /* pulse pattern */
    PULSE_PAT_WHITE,
    PULSE_PAT_BLUE,
    PULSE_PAT_RED,
    PULSE_PAT_AMBER,

    /* transition pattern */
    TRANS_PAT_RED_1,
    TRANS_PAT_RED_2,
    TRANS_PAT_RED_3,

    TRANS_PAT_BLUE_1,
    TRANS_PAT_BLUE_2,
    TRANS_PAT_BLUE_3,
    
    QUICK_FLASH_2S_PAT_WHITE_THEN_PULSE_WHITE,
    QUICK_FLASH_2S_PAT_GREEN_THEN_PULSE_WHITE,

    TRANS_ON_2_OFF_PAT_WHITE,
    TRANS_ON_2_OFF_PAT_AMBER,
    TRANS_ON_2_OFF_PAT_RED,
    TRANS_ON_2_OFF_PAT_BLUE,
    TRANS_DIM_PAT_WHITE,
    TRANS_DIM_PAT_AMBER,
    TRANS_DIM_PAT_RED,
    TRANS_DIM_PAT_BLUE,
    TRANS_DIM_2_OFF_PAT_WHITE,
    TRANS_DIM_2_OFF_PAT_AMBER,
    TRANS_DIM_2_OFF_PAT_RED,
    TRANS_DIM_2_OFF_PAT_BLUE,

    PAT_MAX_NUMBER
}ePattern;

typedef enum eStripPattern
{
    SPAT_GOOGLE_HOTWORD = 0,
    SPAT_GOOGLE_THINKING,
    SPAT_GOOGLE_LISTENING,
    SPAT_GOOGLE_RESPONDING,
    SPAT_GOOGLE_OFF,

    SPAT_MAX_NUMBER
}eStripPattern;

typedef enum eIoeAutoPatt
{
    IOE_AUTO_PATT_STBL = 0,
    IOE_AUTO_PATT_ERROR,
    IOE_AUTO_PATT_MAX,
}eIoeAutoPatt;

typedef struct _tLed_Ind
{
    ledMask     leds;
    ePattern    patternId;
} tLedInd;

/* Please add LED indicaitons according to the UI design of your project. */
typedef enum _eLedIndID
{
    /* Product status */
    LED_IND_ID_POWERING_UP = 0,
    LED_IND_ID_POWERING_DOWN,
    LED_IND_ID_POWERED_ON,
    LED_IND_ID_PRODUCT_IDLE,
    
    LED_IND_ID_BT_MODE,
    LED_IND_ID_WIFI_MODE,
    LED_IND_ID_AUXIN_MODE,
    LED_IND_ID_SPDIF_MODE,
    LED_IND_ID_DSP_TUNING_MODE,
    LED_IND_ID_FAST_BOOT_MODE,

    /* Network Connectivity */
    LED_IND_ID_ETH_CONNECTED,
    LED_IND_ID_ETH_CONNECTED_IDLE,
    LED_IND_ID_WIFI_CONNECTING,
    LED_IND_ID_WIFI_SETUP_IN_PROGRESS,
    LED_IND_ID_WIFI_SIG_STRENGTH_STRONG,/*  X > 25%*/
    LED_IND_ID_WIFI_SIG_STRENGTH_STRONG_IDLE,
    LED_IND_ID_WIFI_SIG_STRENGTH_NORMAL,/*  10 < X <= 25% */
    LED_IND_ID_WIFI_SIG_STRENGTH_NORMAL_IDLE,
    LED_IND_ID_WIFI_SIG_STRENGTH_WEAK,  /* X<= 10% */
    LED_IND_ID_WIFI_SIG_STRENGTH_WEAK_IDLE,
    LED_IND_ID_WIFI_ERROR,
    LED_IND_ID_WIFI_ERROR_IDLE,
    LED_IND_ID_WIFI_UNCONFIGURED,
    LED_IND_ID_WIFI_UNCONFIGURED_IDLE,
    LED_IND_ID_BLUETOOTH,
    LED_IND_ID_BLUETOOTH_IDLE,

    /* BT Connectivity */
    LED_IND_ID_BLE_PAIRING_ENABLED,
    LED_IND_ID_BT_PAIRING_SUCCESSFULL,
    LED_IND_ID_BT_PAIRING_FAIL,

    /* Other status */
    LED_IND_ID_SW_UPDATING,
    LED_IND_ID_PROD_FW_ERROR,
    LED_IND_ID_HW_OVER_HEAT,
    LED_IND_ID_FACTORY_RESET_TRIGGERED,
    LED_IND_ID_SHORT_PRESS,
    LED_IND_ID_CONTINUOUS_PRESS, /* only for volume key */
    LED_IND_ID_LP_AND_VLP,       /* for long press and very long press */
    LED_IND_ID_SHOP_MODE_POWERING_UP_PROD,
    LED_IND_ID_SHOP_MODE_POWERING_UP_CONN,
    LED_IND_ID_SHOP_MODE_PROD,
    LED_IND_ID_SHOP_MODE_CONN,
    
    LED_IND_ID_CONN_TRANS_ON_2_OFF_WHITE,
    LED_IND_ID_CONN_TRANS_ON_2_OFF_AMBER,
    LED_IND_ID_CONN_TRANS_ON_2_OFF_RED,
    LED_IND_ID_CONN_TRANS_ON_2_OFF_BLUE,
    LED_IND_ID_CONN_TRANS_DIM_WHITE,
    LED_IND_ID_CONN_TRANS_DIM_AMBER,
    LED_IND_ID_CONN_TRANS_DIM_RED,
    LED_IND_ID_CONN_TRANS_DIM_BLUE,
    LED_IND_ID_CONN_TRANS_DIM_2_OFF_WHITE,
    LED_IND_ID_CONN_TRANS_DIM_2_OFF_AMBER,
    LED_IND_ID_CONN_TRANS_DIM_2_OFF_RED,
    LED_IND_ID_CONN_TRANS_DIM_2_OFF_BLUE,
    LED_IND_ID_PROD_TRANS_DIM_2_OFF_WHITE,
    LED_IND_ID_PROD_TRANS_OFF,
    LED_IND_ID_CONN_OFF,
    LED_IND_ID_PROD_OFF,
    LED_IND_ID_ALL_TRANS_OFF,

    LED_IND_ID_ALL_OFF,
    LED_IND_ID_MAX
}eLedIndID;

/* Please config below LED mask according to the HW design of your project. */
typedef enum _eLedMask
{
    LED_MASK_MIN           = 0,

    LED_MASK_GOOGLE1_WHT   = 1 << (LED1_GOOGLE1_WHT),
    LED_MASK_GOOGLE2_WHT   = 1 << (LED2_GOOGLE2_WHT),
    LED_MASK_GOOGLE3_WHT   = 1 << (LED3_GOOGLE3_WHT),
    LED_MASK_GOOGLE4_WHT   = 1 << (LED4_GOOGLE4_WHT),

    LED_MASK_CONN_RED      = 1 << (LED5_CONN_RED),
    LED_MASK_CONN_ORG      = 1 << (LED6_CONN_ORG),
    LED_MASK_CONN_BLUE     = 1 << (LED7_CONN_BLUE),
    LED_MASK_CONN_WHT      = 1 << (LED8_CONN_WHT),

    LED_MASK_PROD_RED      = 1 << (LED9_PROD_RED),
    LED_MASK_PROD_WHT      = 1 << (LED10_PROD_WHT),

    LED_MASK_VOL_SENS_EN   = 1 << (LED11_VOL_SENS_EN),

    LED_MASK_MIC_WHT       = 1 << (LED12_MIC_WHT),
    LED_MASK_MIC_ORG       = 1 << (LED13_MIC_ORG),

    LED_MASK_GOOGLE1      = (LED_MASK_GOOGLE1_WHT),
    LED_MASK_GOOGLE2      = (LED_MASK_GOOGLE2_WHT),
    LED_MASK_GOOGLE3      = (LED_MASK_GOOGLE3_WHT),
    LED_MASK_GOOGLE4      = (LED_MASK_GOOGLE4_WHT),

    LED_MASK_GOOGLE_LEDS  = (LED_MASK_GOOGLE1 | LED_MASK_GOOGLE2 | LED_MASK_GOOGLE3 | LED_MASK_GOOGLE4),
    LED_MASK_MIC          = (LED_MASK_MIC_ORG | LED_MASK_MIC_WHT),


    LED_MASK_LED0_RED           = 1<<(LED0_RED),        //TBD
    LED_MASK_LED1_GREEN         = 1<<(LED1_GREEN),      //TBD
    LED_MASK_LED2_BLUE          = 1<<(LED2_BLUE),       //TBD
    LED_MASK_LED3_RED           = 1<<(LED3_RED),        //TBD
    LED_MASK_LED4_GREEN         = 1<<(LED4_GREEN),      //TBD
    LED_MASK_LED5_BLUE          = 1<<(LED5_BLUE),       //TBD


    LED_MASK_PROD_LEDS    = (LED_MASK_PROD_RED | LED_MASK_PROD_WHT),
    LED_MASK_CONN_LEDS    = (LED_MASK_CONN_WHT | LED_MASK_CONN_BLUE | LED_MASK_CONN_ORG | LED_MASK_CONN_RED),
    LED_MASK_ALL_LEDS     = (LED_MASK_CONN_LEDS | LED_MASK_PROD_LEDS),//0x0000f,//0xfbfff,//15 pin used as a output port

    LED_MASK_MAX
}eLedMask;

#endif

...
#if 0   //(DEBUG_ON_EVK || DEBUG_ON_PRE_ES)
...
#endif
#ifdef BRINGUP_DEBUG
                PersistantObj_RefreshTick((cPersistantObj*)me, 500); // LED indicate HW error for 500ms seconds then power on system.
#else
...
QState MainApp_Active(cMainApp * const me, QEvt const * const e)
{
    ...
    switch (e->sig)
    {
        ...
        case SYSTEM_MODE_RESP_SIG:
        {
            ...
            AudioSrv_SetAudio(AMP_SLEEP_MODE_ID, FALSE, /* NO USED*/0, /*not-used*/0); 
#endif
            return Q_HANDLED();
        }

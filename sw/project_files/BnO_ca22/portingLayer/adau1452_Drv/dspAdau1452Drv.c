...
#define DspDrv1452_DEBUG_ENABLE
#ifndef DspDrv1452_DEBUG_ENABLE
#undef TP_PRINTF
#define TP_PRINTF(...)
#endif
...
void DSPDrv1452_WriteDcValue32_0(cDSPDrv1452 *me, uint16 reg_addr, int32 data);
void DSPDrv1452_WriteValue5_23(cDSPDrv1452 *me, uint16 reg_addr, float fValue);
float DSPDrv1452_ReadValue5_23(cDSPDrv1452 *me, uint16 reg_addr);


static void DspDrv1452_CalculateFirstOrder(cDSPDrv1452 *me,uint16 freq, uint8* out);
static void DspDrv1452_CalculateSecondOrder(cDSPDrv1452 *me,uint16 freq, double delta, uint8* out);
static float DSPDrv1452_DataToFloat(cDSPDrv1452 *me, uint32 data, uint8 fmt_num, uint8 fmt_frac);

static int16 *pSettData;
static cDSPDrv1452 * pDspObj;
...
{
    {&DspDrv1452_PowerOnSeq0,  100}, //turn off VCC, disable I2S
    {&DspDrv1452_PowerOnSeq1,  100}, //turn on VCC, enable I2S
    {&DspDrv1452_PowerOnSeq2,  1},   //dummy
    {&DspDrv1452_InitSection0, 50},
    {&DspDrv1452_InitSection1, 100},
    {&DspDrv1452_InitSection2, 50},
};
...
void DSPDrv1452_Mute(cDSPDrv1452 *me, eAudioMuteType muteType, BOOL muteEnable)
{
    ...
    DspDrv1452_I2cRead(me, PLAYBACK_LR_LEFT_ADDR,  MIXER_REG_LENGTH, (uint8*)&mixer_ll);
    DspDrv1452_I2cRead(me, PLAYBACK_LR_RIGHT_ADDR, MIXER_REG_LENGTH, (uint8*)&mixer_lr);
    ...
    case TRUE:
      {
        ...
        temp[2] = mixer_ll;
        DspDrv1452_I2cWrite(me, sizeof(temp), temp);
...
static void DSPDrv1452_SetVol(cDSPDrv1452 *me, uint8 vol)
{
 ...
 printf("vol= %d\r\n", vol);
#ifdef NEO
    uint8 vol_iic[ADAU1452_UPDATE_VOLUME_LEN] = {0,0,0,0,0,0}; /* write DSP register */
    uint32 reg_addr= ADAU1452_VOLUME_CONTROL_ADDR;
    ...
    DspDrv1452_I2cWrite(me, ADAU1452_UPDATE_VOLUME_LEN, (uint8*)vol_iic);
#endif
}
...
//            DSPDrv1452_SetVol(me,volume);
//          }
//          else
//          {
//            DSPDrv1452_SetVol(me,0);
//          }
//        }
//        break;
...
//          DspDrv1452_SetRGC(me, freq, slope, enable);
//        }
//        break;
...
//          DspDrv1452_SetUserLp(me, freq, slope, enable);
//        }
//        break;
...
//          DspDrv1452_SetUserHp(me, freq, slope, enable);
//        }
//        break;
//      case DSP_EQ_CTRL_PEQ1:
//      case DSP_EQ_CTRL_PEQ2:
//      case DSP_EQ_CTRL_PEQ3:
//        DspDrv1452_SetPeq(me, dspSettId, i, enable);
//        break;
//      case DSP_PHASE_SETT_ID:
//        DspDrv1452_SetPhase(me, (pSettData[ctrIdEQIdMap[i].dbIndex])/VALUE_MAGNIFICATION);
//        break;
//      case DSP_POLARITY_SETT_ID:
//        DspDrv1452_SetPolarity(me, (ePolarityType)((pSettData[ctrIdEQIdMap[i].dbIndex])/VALUE_MAGNIFICATION));
//        break;
//      case DSP_TUNNING_SETT_ID:
//        DspDrv1452_SetTuning(me, (eTuningRange)((pSettData[ctrIdEQIdMap[i].dbIndex])/VALUE_MAGNIFICATION));
//        break;
//      default:break;
//    }
//}
      ...
      case DSP_VOLUME_SETT_ID:
        {
            ...
            DSPDrv1452_SetVol(me,volumeLevel);
        }
        break;
      case DSP_EQ_CTRL_RGC:
        {
...
{
    uint8 buffer[2+ADAU1452_MAX_TX_BYTES];
    ASSERT(size<=ADAU1452_MAX_TX_BYTES); /* Note: the size should not more than ADAU1451_REGISTER_LEN. */

    buffer[0]=(regAddr>>8);
    buffer[1]=(regAddr&0xff);
    memcpy(&buffer[2], pData, size);
    DspDrv1452_I2cWrite(me, (2+size), (uint8*)buffer);
}
...
//    DspDrv1452_I2cRead(me, POLARITY_CHANNEL_CTRL_ADDRESS, 4, (uint8*)&polarityChannel);
    if(polarityChannel[3] == POLARITY_CHANNEL_0)
    {
        polarityChannel[3] = POLARITY_CHANNEL_1;
//        DspDrv1452_SafeLoadData(me, 1, temp, POLARITY_CHANNEL_1_ADDRESS);
    }
    else if(polarityChannel[3] == POLARITY_CHANNEL_1)
    {
        polarityChannel[3] = POLARITY_CHANNEL_0;
//        DspDrv1452_SafeLoadData(me, 1, temp, POLARITY_CHANNEL_0_ADDRESS);
    }
//    DspDrv1452_SafeLoadData(me, 1, polarityChannel, POLARITY_CHANNEL_CTRL_ADDRESS);
}


static void DspDrv1452_SetPhaseCB(void * pCbPara)
{
    uint8    delayChannel[4] = {0};

//    DspDrv1452_I2cRead(pDspObj, DELAY_CHANNEL_CTRL_ADDRESS, 4, (uint8*)&delayChannel);
    if(isPhaseInited)
    {
    ...
    isPhaseInited = TRUE;
//    DspDrv1452_SafeLoadData(pDspObj, 1, delayChannel, DELAY_CHANNEL_CTRL_ADDRESS);
...
//    DspDrv1452_I2cRead(pDspObj, DELAY_CHANNEL_CTRL_ADDRESS, 4, (uint8*)&delayChannel);
    if(delayChannel[3] == DELAY_CHANNEL_0)
    {
//        DspDrv1452_SafeLoadData(me, 1, temp, DELAY_CHANNEL_1_ADDRESS);
    }
    else if(delayChannel[3] == DELAY_CHANNEL_1)
    {
//        DspDrv1452_SafeLoadData(me, 1, temp, DELAY_CHANNEL_0_ADDRESS);
    }
    Timer_StartTimer(600,&phaseTimer, DspDrv1452_SetPhaseCB, NULL);
}
...
{
    //TP_PRINTF("DspDrv1452_InitSection0\r\n");
    cDSPDrv1452 * me = (cDSPDrv1452*)p;
...
{
    //TP_PRINTF("DspDrv1452_InitSection1\r\n");
    cDSPDrv1452* me = (cDSPDrv1452*)p;
/*
...
{
    //TP_PRINTF("DspDrv1452_InitSection2\r\n");
    cDSPDrv1452* me = (cDSPDrv1452*)p;
...
static void DspDrv1452_CalFiltParam(tDspFiltRawParam in, uint8* out)
{
    ...
    if(in.en == FALSE)
    {
        ...
        DspDrv1452_FormatFiltData(temp, out, EQ_CTRL_RAM_SLOT_NUM);
        return;
    }
    else if(in.q == 0)
    {
...
void DspDrv1452_SetI2Smode(cDSPDrv1452 *me, BOOL isMaster)
{
    ...
    //read
    uint8   data_read[1] = {0};
    DspDrv1452_I2cRead(me, ADAU1452_I2S_CONTROL_ADDR, sizeof(data_read), data_read);    
    if(isMaster)
    {
    ...
    uint32 reg_addr= ADAU1452_I2S_CONTROL_ADDR;
    data_write[0] = reg_addr >> 8;
    data_write[1] = reg_addr & 0x00FF;
    data_write[2] = data_read[0];
    DspDrv1452_I2cWrite(me, sizeof(data_write), (uint8*)data_write);
}
...
static void DspDrv1452_SetRGC(cDSPDrv1452 *me, uint16 freq, eUserRGCSlope slope, bool en)
{
    ...
    /* cal out the EQ data */
    DspDrv1452_CalFiltParam(rawData, finalData);
        /* check slope */
    switch(slope)
    {
    ...
    for(i = 0; i < NUM_OF_RGC_SET_ADD; i++)
    {
        if(i < currIdx)
        {
            DspDrv1452_SafeLoadData(me,EQ_CTRL_RAM_SLOT_NUM, finalData, regAddress);
        }
        else
        {
            DspDrv1452_SafeLoadData(me, EQ_CTRL_RAM_SLOT_NUM, eqBypassData, regAddress);
        }
...
static void DspDrv1452_SetUserLp(cDSPDrv1452 *me, uint16 freq, eUserLpSlope slope, bool en)
{
    ...
    {
        finalData[3] = 0x01;
//        DspDrv1452_SafeLoadData(me, 1, finalData, LP_SWITCH_ADDRESS);
    }
    else
    {
//        DspDrv1452_SafeLoadData(me, 1, finalData, LP_SWITCH_ADDRESS);
    ...
    switch(slope)
    {
        case USER_LP_SLOPE_6DB:
            ...
            DspDrv1452_CalculateFirstOrder(me,freq,finalData);
//            DspDrv1452_SafeLoadData(me, EQ_CTRL_RAM_SLOT_NUM, finalData, LP_CTRL_ADDRESS);
            memset(finalData,0x00,EQ_CTRL_RAM_BYTE_NUM);
            DspDrv1452_FormatFiltData(bypassData, finalData, EQ_CTRL_RAM_SLOT_NUM);
//            DspDrv1452_SafeLoadData(me, EQ_CTRL_RAM_SLOT_NUM, finalData, LP_CTRL_ADDRESS+5);
            break;
        case USER_LP_SLOPE_12DB:
            delta = delta2;
            DspDrv1452_CalculateSecondOrder(me, freq, delta, finalData);
//            DspDrv1452_SafeLoadData(me, EQ_CTRL_RAM_SLOT_NUM, finalData, LP_CTRL_ADDRESS);
            memset(finalData,0x00,EQ_CTRL_RAM_BYTE_NUM);
            DspDrv1452_FormatFiltData(bypassData, finalData, EQ_CTRL_RAM_SLOT_NUM);
//            DspDrv1452_SafeLoadData(me, EQ_CTRL_RAM_SLOT_NUM, finalData, LP_CTRL_ADDRESS+5);
...
//            DspDrv1452_SafeLoadData(me, EQ_CTRL_RAM_SLOT_NUM, finalData, LP_CTRL_ADDRESS);
            ...
            delta = delta4_2;
            DspDrv1452_CalculateSecondOrder(me, freq, delta, finalData);
//            DspDrv1452_SafeLoadData(me, 5, finalData, LP_CTRL_ADDRESS+5);
            break;
        default:
            ASSERT(0);
            break;
...
static void DspDrv1452_SetTuning(cDSPDrv1452 *me, eTuningRange range)
{
    ...
    /* cal out the EQ data */
    DspDrv1452_CalFiltParam(rawData, finalData);
//    DspDrv1452_SafeLoadData(me, EQ_CTRL_RAM_SLOT_NUM, finalData, TUNNING_ADDRESS);
}
...
void DSPDrv1452_GetSigLevel(cDSPDrv1452 *me, float *fSigLev)
{
    int8 result = -1;
    ASSERT(me && me->isCreated);
...
//    DspDrv1452_I2cRead(me, ADAU1452_AUXIN_SIG_DETECT_ADDR, sizeof(data), data);
//    float res = 0.0;
    
//    /* RMS table of ADAU1452 can only store 5.23 float (around -16.0~16.0), it can not report 
//     * correct dB (-90~+6). Thus we let RMS table divide by 10, the resul tis -9.0~+6.0).
//     */
//    *fSigLev = (10.0*DSPDrv1452_DataToFloat(me, UINT32_BYTE_INV(data), 5,23 )); //format 5.23
...
{
//    DSPDrv1452_Mute(me, AUDIO_AMP_SOFT_MUTE, TRUE);
...
//    }
    ...
    if(ctrData)
    {
        DspDrv1452_I2cWrite(me, SOURCE_SELECT_REGISTER_LEN, (uint8*)ctrData);
    }

}


void DSPDrv1452_SetAuxinMute(cDSPDrv1452 *me, bool auxin_mute)
{
    ...
    DspDrv1452_I2cWrite(me, SOURCE_LINEIN_MUTE_CONTROL_REGISTER_LEN, (uint8*)ctrData);
#endif
}
...
void DSPDrv1452_SetToneTouchEnable(cDSPDrv1452 *me, bool enable)
{
    ...
    DspDrv1452_I2cWrite(me, TONE_TOUCH_ENABLE_CONTROL_REGISTER_LEN, (uint8*)ctrData);
#endif
}
...
{
    TP_PRINTF("DSPDrv1452_SetSpeakerPosition: position=%d\r\n", position);
    const uint8 * ctrData = NULL;
#ifdef NEO
    switch (position)
    {
        ...
        {
            ctrData = ADAU1452_SPEAKER_POSITION_FREE;
            break;
        }
        case SPEAKER_POSITION_WALL:
        {
            ctrData = ADAU1452_SPEAKER_POSITION_WALL;
            break;
        }
        case SPEAKER_POSITION_CORNER:
        {
            ctrData = ADAU1452_SPEAKER_POSITION_CORNER;
            break;
        }
        default:
        {
    ...
    if(ctrData)
    {
        DspDrv1452_I2cWrite(me, SPEAKER_POSITION_SWITCH_REGISTER_LEN, (uint8*)ctrData);
    }
#endif
}
 ...
 * @return     float float version
 */
void DSPDrv1452_GetDspVer(cDSPDrv1452 *me, float *fVerRet)
{
    ASSERT(me && me->isCreated);
    uint8   data[DSP_VER_REGISTER_LEN] = {0};

    //DspDrv1452_I2cRead(me, DSP_VER_REGISTER, sizeof(data), data);

    float ver= DSPDrv1452_DataToFloat(me, UINT32_BYTE_INV(data), 5, 23); //format 5.23
    *fVerRet = ver;    
}
 ...
 */
void DSPDrv1452_I2cEnable(cDSPDrv1452 *me, bool i2cEnable)
{
 ...
 *
 */
/*static */uint32 DSPDrv1452_FloatTo5_23Data(cDSPDrv1452 *me, float float_val)
{
  ...
  {
      //ASSERT(0);
      TP_PRINTF("\r\n*** DSPDrv1452_FloatTo5_23Data: invalid floating nubmer %f\r\n\r\n", float_val );
      return 0;
  }
...
void DSPDrv1452_WriteDcValue32_0(cDSPDrv1452 *me, uint16 reg_addr, int32 data)
{
    //TP_PRINTF("DSPDrv1452_WriteDcValue32_0: reg=%d, data=%d\r\n", reg_addr, data);   
    ...
    DSPDrv1452_I2cWrite(me, DC_REGISTER_LEN, (uint8*)data_iic);
}
...
void DSPDrv1452_WriteValue5_23(cDSPDrv1452 *me, uint16 reg_addr, float fValue)
{
    uint32 data= DSPDrv1452_FloatTo5_23Data(me, fValue);
    //TP_PRINTF("DSPDrv1452_WriteValue5_23: reg=%d, dValue=%f => data=%d\r\n", reg_addr, fValue, data);
...
float DSPDrv1452_ReadValue5_23(cDSPDrv1452 *me, uint16 reg_addr)
{
    ...
    ASSERT(me->isCreated);
    DspDrv1452_I2cRead(me, reg_addr, sizeof(data), data);
    fValue= DSPDrv1452_DataToFloat(me, UINT32_BYTE_INV(data), 5, 23); //format 5.23
...
bool DSPDrv1452_IsPllLock(cDSPDrv1452 *me)
{
    uint8 data[6] = {0}; 
    ASSERT(me->isCreated);
    DspDrv1452_I2cRead(me, PLLCTRLREGISTER_ADDR, sizeof(data), data);

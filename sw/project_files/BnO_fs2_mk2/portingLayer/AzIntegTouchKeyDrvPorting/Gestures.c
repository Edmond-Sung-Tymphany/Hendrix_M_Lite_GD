...
#define TOUCH_COORDINATE_SIZE        6
#define TOUCH_COORDINATE_HEADER      ((TOUCH_COORDINATE_SIZE << 8) | 17 )

#define inner_ring          (double)2.09
#define outer_ring          (double)2.79

#define BUTTON_RANGE        280   //56*5mm
Point_2D_raw mbtn_1 =
{
    .x = 276,
    .y = 287
...
Point_2D_raw touch_mic =
{
    .x = 892,
    .y = 144
};
Point_2D_raw touch_center =
{
    .x = 892,
    .y = 892
};
 ...
 * @retval	None
 */
eKeyID touch_key_distinguish(void)
{
    uint16_t touch_x = g_SwipeAxis.start_raw.x;
    uint16_t touch_y = g_SwipeAxis.start_raw.y;

    uint16_t dis_mbtn_1 = (uint16_t)sqrt(pow((touch_x - mbtn_1.x),2)+pow((touch_y - mbtn_1.y),2));
    if(dis_mbtn_1 <= BUTTON_RANGE)
    {
        ATOUCH_PRINTF("\r\n********mybutton 1**********\r\n");
        return TOUCH_TAP_MBUTTON_1;
    }

    uint16_t dis_mbtn_2 = (uint16_t)sqrt(pow((touch_x - mbtn_2.x),2)+pow((touch_y - mbtn_2.y),2));
    if(dis_mbtn_2 <= BUTTON_RANGE)
    {
        ATOUCH_PRINTF("\r\n********mybutton 2**********\r\n");
        return TOUCH_TAP_MBUTTON_2;
    }

    uint16_t dis_mbtn_3 = (uint16_t)sqrt(pow((touch_x - mbtn_3.x),2)+pow((touch_y - mbtn_3.y),2));
    if(dis_mbtn_3 <= BUTTON_RANGE)
    {
        ATOUCH_PRINTF("\r\n********mybutton 3**********\r\n");
        return TOUCH_TAP_MBUTTON_3;
    }

    uint16_t dis_mbtn_4 = (uint16_t)sqrt(pow((touch_x - mbtn_4.x),2)+pow((touch_y - mbtn_4.y),2));
    if(dis_mbtn_4 <= BUTTON_RANGE)
    {
        ATOUCH_PRINTF("\r\n********mybutton 4**********\r\n");
        return TOUCH_TAP_MBUTTON_4;
    }

    uint16_t dis_touch_mic = (uint16_t)sqrt(pow((touch_x - touch_mic.x),2)+pow((touch_y - touch_mic.y),2));
    if(dis_touch_mic <= BUTTON_RANGE)
    {
        ATOUCH_PRINTF("\r\n********touch mic**********\r\n");
        return TOUCH_TAP_MIC;
    }

    uint16_t dis_touch_center = (uint16_t)sqrt(pow((touch_x - touch_center.x),2)+pow((touch_y - touch_center.y),2));
    if(dis_touch_center <= BUTTON_RANGE)
    {
        ATOUCH_PRINTF("\r\n********touch center**********\r\n");
        return TOUCH_TAP_KEY;
    }
...
void get_gesture(uint8_t* xy_info, bool noise)
{
	...
	eKeyID touch_key_id = INVALID_KEY;
    uint16_t touch_coor_data[3] = {0};
    touch_coor_data[0] = TOUCH_COORDINATE_HEADER;
	...
	switch(gesture_byte)
	{
		...
		case TAP:
			...
			if(!noise)
            {
                touch_key_id = touch_key_distinguish();
                switch(touch_key_id)
                {
                    case TOUCH_TAP_MBUTTON_1:
                    case TOUCH_TAP_MBUTTON_2:
                    case TOUCH_TAP_MBUTTON_3:
                    case TOUCH_TAP_MBUTTON_4:
                    case TOUCH_TAP_MIC:
                        KeySrv_SendKeyEvt_Direct(KEY_EVT_SHORT_PRESS, touch_key_id);
                        ATOUCH_PRINTF("\r\n-------send shortpress key-------\n\r\n");
                        break;
                    case TOUCH_TAP_KEY:
                        doubleTapAnalyse();
                ...
                touch_coor_data[1] = g_SwipeAxis.start_raw.x;
                touch_coor_data[2] = g_SwipeAxis.start_raw.y;
                AseNgSrv_SendTunnel(touch_coor_data, TOUCH_COORDINATE_SIZE);
                    ...
                    if(!noise)
                    {
                        ...
                        */
                        ...
                        if( (currTimeMs - lastHoldTapTimeMs) > HOLD_FILTER_TIME_MS )
                        {
                            ...
                            if(touch_key_id != INVALID_KEY)
                            {
                                KeySrv_SendKeyEvt_Direct(KEY_EVT_HOLD, touch_key_id);
                                ATOUCH_PRINTF("\r\n--------send hold key-------\n\r\n");
                            }
                        }

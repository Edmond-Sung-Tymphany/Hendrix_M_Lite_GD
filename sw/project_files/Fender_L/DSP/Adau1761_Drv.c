...
#if defined(DSP_BYPASS_MODE_ENABLE)
#include "./Fender_Watermelon_BP_IC_1.h"
#include "./Fender_Watermelon_BP_IC_1_REG.h"
#include "./Fender_Watermelon_BP_IC_1_PARAM.h"
#elif defined(DSP_FULL_POWER_OUTPUT)
#include "./Fender_Watermelon_SineWave_IC_1.h"
#include "./Fender_Watermelon_SineWave_IC_1_REG.h"
#include "./Fender_Watermelon_SineWave_IC_1_PARAM.h"
#else
#include "./Fender_Watermelon_IC_1.h"
#include "./Fender_Watermelon_IC_1_REG.h"
#include "./Fender_Watermelon_IC_1_PARAM.h"
#endif
#else
// dsp files
...
#else
#include "./Fender_Large_IC_1.h"
//#include "./Fender_Large_IC_1_REG.h"
#include "./Fender_Large_IC_1_PARAM.h"
#endif
#endif  // WOOFER_TWEETER_SWAP_ENABLE
...
{
    {&Adau1761Drv_InitStage0, 100},
    {&Adau1761Drv_InitStage1, 50},
};
...
#ifdef LINE_IN_ENABLE_DIFFERENTIAL_PATH
...
#else
/* NOTE : according to the Fender-Watermelon H/W, LIN-N & RIN-N is always setting to MUTE */
    if( analog_in == DSP_ANALOG_AUX_IN )
    {
        // mute line-in(P-Channel only, N-Channel is always MUTE, needn't to update) path
        // line-in setting : 0x01:mute, 0x11:-12dB, 0x21:-9dB, 0x31:-6dB, 0x41:-3dB, 0x51:0dB, 0x61:+3dB, 0x71:+6dB
        reg_data = 0x01;    // line-in -> mute
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_LEFT_CTRL_0_IC_1_ADDR, REG_RECORD_MIXER_LEFT_CTRL_0_IC_1_BYTE, (uint8_t *)(&reg_data));
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_RIGHT_CTRL_0_IC_1_ADDR, REG_RECORD_MIXER_RIGHT_CTRL_0_IC_1_BYTE, (uint8_t *)(&reg_data));
        // demute AUX in path
        reg_data = 0x05;    // aux in -> 0dB
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_LEFT_CTRL_1_IC_1_ADDR, REG_RECORD_MIXER_LEFT_CTRL_1_IC_1_BYTE, (uint8_t *)(&reg_data));
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_RIGHT_CTRL_1_IC_1_ADDR, REG_RECORD_MIXER_RIGHT_CTRL_1_IC_1_BYTE, (uint8_t *)(&reg_data));
    }
    else
    {
        // mute AUX in path
        reg_data = 0x00;    // aux in -> mute
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_LEFT_CTRL_1_IC_1_ADDR, REG_RECORD_MIXER_LEFT_CTRL_1_IC_1_BYTE, (uint8_t *)(&reg_data));
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_RIGHT_CTRL_1_IC_1_ADDR, REG_RECORD_MIXER_RIGHT_CTRL_1_IC_1_BYTE, (uint8_t *)(&reg_data));
        // demute line-in(P-Channel only, N-Channel is always MUTE, needn't to update) path
        // line-in setting : 0x01:mute, 0x11:-12dB, 0x21:-9dB, 0x31:-6dB, 0x41:-3dB, 0x51:0dB, 0x61:+3dB, 0x71:+6dB
        reg_data = 0x41;    // line-in -> -3dB
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_LEFT_CTRL_0_IC_1_ADDR, REG_RECORD_MIXER_LEFT_CTRL_0_IC_1_BYTE, (uint8_t *)(&reg_data));
        Adau1761Drv_I2CWrite(REG_RECORD_MIXER_RIGHT_CTRL_0_IC_1_ADDR, REG_RECORD_MIXER_RIGHT_CTRL_0_IC_1_BYTE, (uint8_t *)(&reg_data));
    }
#endif
...
bool Adau1761Drv_MusicDetected(void)
{
	...
	// when using sigma studio tuning, don't enter auto standby
    if( ! p_adau1761_drv->i2cEnable )
        return TRUE;
...
/* mute control on woofer-left channel */
void Adau1761Drv_Mute_WL(bool mute)
{
    uint16_t reg_addr;
    uint8_t reg_data[4];

    reg_addr = MOD_MUTEWL_MUTENOSLEWALG1MUTE_ADDR;

    reg_data[0] = reg_data[2] = reg_data[3] = 0;
...
/* mute control on woofer-right channel */
void Adau1761Drv_Mute_WR(bool mute)
{
    uint16_t reg_addr;
    uint8_t reg_data[4];

    reg_addr = MOD_MUTEWR_MUTENOSLEWALG2MUTE_ADDR;
    reg_data[0] = reg_data[2] = reg_data[3] = 0;
...
/* mute control on tweeter-left channel */
void Adau1761Drv_Mute_TL(bool mute)
{
    uint16_t reg_addr;
    uint8_t reg_data[4];

    reg_addr = MOD_MUTETL_MUTENOSLEWALG3MUTE_ADDR;
...
/* mute control on tweeter-right channel */
void Adau1761Drv_Mute_TR(bool mute)
{
    uint16_t reg_addr;
    uint8_t reg_data[4];

    reg_addr = MOD_MUTETR_MUTENOSLEWALG4MUTE_ADDR;
...
void Adau1761Drv_EnableShapeEQ(bool enable)
{
    ...
    reg_addr = MOD_USERSPECIALFEATURE_SHARPNORMAL_STEREOSWSLEW_ADDR;

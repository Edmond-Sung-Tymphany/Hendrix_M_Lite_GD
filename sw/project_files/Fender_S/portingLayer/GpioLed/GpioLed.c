...
{
#ifdef HW_ES2
    { RCC_AHBPeriph_GPIOB, GPIOB, GPIO_Pin_12 },    // LED1 : Talk
    { RCC_AHBPeriph_GPIOB, GPIOB, GPIO_Pin_13 },    // LED2 : POWER
    { RCC_AHBPeriph_GPIOA, GPIOA, GPIO_Pin_8 },     // LED3 : Charging(portalbe) / RCA(large)
    { RCC_AHBPeriph_GPIOA, GPIOA, GPIO_Pin_11 },    // LED4 : BT/Paring
#else
    { RCC_AHBPeriph_GPIOA, GPIOA, GPIO_Pin_11 },    // LED1 : Talk
    { RCC_AHBPeriph_GPIOB, GPIOB, GPIO_Pin_13 },    // LED2 : POWER
    { RCC_AHBPeriph_GPIOA, GPIOA, GPIO_Pin_8 },     // LED3 : Charging(portalbe) / RCA(large)
    { RCC_AHBPeriph_GPIOB, GPIOB, GPIO_Pin_12 },    // LED4 : BT/Paring
#endif
};

static GLED_CONTROLLER_t gled_controller[GLED_ID_END];

static GLED_CONTROLLER_t gled_prev_controller[GLED_ID_END];

void GLED_OnOff(GLED_ID_t gled_id, uint8_t on)
{
    if( on )
    {
...
void GLED_ALLLEDOff(void)
{
    GLED_ID_t gled_id;

    for(gled_id=GLED_ID_START; gled_id<GLED_ID_END; gled_id++)
    {
        if (GLED_ID_CHARGING != gled_id)
        {
            GLED_OnOff(gled_id, FALSE);
        }            
    }            
}
...
void GLED_ErrorMode(void)
{
    uint8 i;

    for (i=0; i<GLED_ID_END; i++)
    {
        gled_prev_controller[i].gled_id = gled_controller[i].gled_id;
        gled_prev_controller[i].gled_mode = gled_controller[i].gled_mode;
        gled_prev_controller[i].cur_tick = gled_controller[i].cur_tick;
        gled_prev_controller[i].repeat = gled_controller[i].repeat;
        gled_prev_controller[i].finally_on = gled_controller[i].finally_on;
        gled_prev_controller[i].cur_on = gled_controller[i].cur_on;
    }

    for (i=0; i<GLED_ID_END; i++)
    {
        GLED_SetupMode((GLED_ID_t)i, GLED_MODE_FLASH, GPIO_LED_ALWAYS_FLASH);
    }
}

void GLED_RecoverMode(void)
{
    uint8 i;

    for (i=0; i<GLED_ID_END; i++)
    {
        gled_controller[i].gled_id = gled_prev_controller[i].gled_id;
        gled_controller[i].gled_mode = gled_prev_controller[i].gled_mode;
        gled_controller[i].cur_tick = gled_prev_controller[i].cur_tick;
        gled_controller[i].repeat = gled_prev_controller[i].repeat;
        gled_controller[i].finally_on = gled_prev_controller[i].finally_on;
        gled_controller[i].cur_on = gled_prev_controller[i].cur_on;
    }
}

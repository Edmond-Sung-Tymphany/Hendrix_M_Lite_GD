...
#ifdef HAS_PWR_IO_EXPANDER
static cIoExpanderDrv PwrIoeDrv;
#endif
...
void SystemDrv_CheckBatteryChargeStatus(void)
{
    ...
    {
        system_power_status.loopCount = 0;
        if (GpioDrv_ReadBit(&powerGpioDrv, GPIO_IN_BAT_CHG_STATUS1))
        {
...
{
#ifdef HENDRIX_M_ES1
    return  FALSE;               //can't detect ntc value while using battery as power source;
#else
...
//            ADCDrv_StartScanning(&adcDrv);    // let the other xxx_Ctor(e.g adc key) to start scanning, don't scan on powering up stage
            ...
            {
                tIoeLedDevice *pPwrLedDevice = (tIoeLedDevice*)pPowerDev;
                IoExpanderDrv_Ctor_aw9523(&PwrIoeDrv, pPwrLedDevice);
                break;
            }
#endif
            default:
...
#ifdef HAS_PWR_IO_EXPANDER
void SystemDrv_Ioe_Set_by_ID(cIoExpanderDrv* drv, uint8 index)
{
    tIoeLedDevice *pPwrLedDevice = (tIoeLedDevice*)getDevicebyIdAndType(POWER_DEV_ID, IO_EXPANDER_DEV_TYPE, NULL);
    uint8 j = 0;
    for ( ; j < pPwrLedDevice->ledNum; j++)
    {
        if(pPwrLedDevice->pIoExpanderLedMap[j].ledID == index)
        {
            uint8 port = pPwrLedDevice->pIoExpanderLedMap[j].port;
            uint8 pin = pPwrLedDevice->pIoExpanderLedMap[j].pin;
            IoExpanderDrv_SetGpio_aw9523(drv, port, pin);
        }
    }
}

void SystemDrv_Ioe_Clear_by_ID(cIoExpanderDrv* drv, uint8 index)
{
    tIoeLedDevice *pPwrLedDevice = (tIoeLedDevice*)getDevicebyIdAndType(POWER_DEV_ID, IO_EXPANDER_DEV_TYPE, NULL);
    uint8 j = 0;
    for ( ; j < pPwrLedDevice->ledNum; j++)
    {
        if(pPwrLedDevice->pIoExpanderLedMap[j].ledID == index)
        {
            uint8 port = pPwrLedDevice->pIoExpanderLedMap[j].port;
            uint8 pin = pPwrLedDevice->pIoExpanderLedMap[j].pin;
            IoExpanderDrv_ClearGpio_aw9523(drv, port, pin);
        }
    }
}
#endif
...
void SystemDrv_BoostEnable(bool enable)
{
    ...
    if (enable)
    {
        BOOST_ENABLE(PwrIoeDrv);
    }
    else
    {
        BOOST_DISABLE(PwrIoeDrv);
    }
#else
    if (enable)
    {
...
#ifdef HAS_AMP_CTRL
void SystemDrv_AMPEnable(bool enable)
{
#ifdef HAS_PWR_IO_EXPANDER
    if (enable)
    {
        AMP_ENABLE(PwrIoeDrv);
    }
    else
    {
        AMP_DISABLE(PwrIoeDrv);
    }
#endif
}
#endif
...
{
    // TRUE 2.5A  FALSE 1.5A
    if (enable)
    {
        system_power_status.chargerCurrentLevel = CHG_CUR_LV_HIGH;
        BAT_CHG_CUR_HIGH(PwrIoeDrv);
    }
    else
    {
        system_power_status.chargerCurrentLevel = CHG_CUR_LV_LOW;
        BAT_CHG_CUR_LOW(PwrIoeDrv);
    }
}
bool SystemDrv_GetBatteryChargeCurrentLevel(void)
{
    return system_power_status.chargerCurrentLevel;
}
...
void SystemDrv_BatteryChargeEnable(bool enable)
{
    ...
    if (enable)
    {
        BAT_CHG_ENABLE(PwrIoeDrv);
    }
    else
    {
        BAT_CHG_DISABLE(PwrIoeDrv);
    }
#else
    if (enable)
    {
...
void SystemDrv_ExternalChargeEnable(bool enable)
{
    ...
    if (enable)
    {
        EX_CHG_CTRL_ENABLE(PwrIoeDrv);
    }
    else
    {
        EX_CHG_CTRL_DISABLE(PwrIoeDrv);
    }
#else
    if (enable)
    {

...
#include "./Adau1761_EqBand.h"
#endif

// dsp files
#ifdef JOPLIN_L
#include "./Joplin_L/ADAU1761_IC_1.h"
#include "./Joplin_L/ADAU1761_IC_1_REG.h"
#include "./Joplin_L/ADAU1761_IC_1_PARAM.h"
#elif defined(JOPLIN_M)
#include "./Joplin_M/ADAU1761_IC_1.h"
#include "./Joplin_M/ADAU1761_IC_1_REG.h"
#include "./Joplin_M/ADAU1761_IC_1_PARAM.h"
#elif defined(JOPLIN_S)
#include "./Joplin_S/ADAU1761_IC_1.h"
#include "./Joplin_S/ADAU1761_IC_1_REG.h"
#include "./Joplin_S/ADAU1761_IC_1_PARAM.h"
#else
...
// dsp v0.1 only support analog in or i2s in.
    ...
    {
        reg_addr = REG_RECORD_VOLUME_CTRL_LEFT_IC_1_ADDR;
        reg_data[0] = 0x22;//Disable line-in
        Adau1761Drv_I2CWrite(reg_addr, 1, (uint8_t *)(&reg_data));
...
#ifdef DSP_TREBLE_BASS_TUNING
    Adau1761Drv_SetEqBands(EQ_BAND1, bass_level);
#endif
}
...
#ifdef DSP_TREBLE_BASS_TUNING
    Adau1761Drv_SetEqBands(EQ_BAND5, treble_level);
#endif
}
...
{
    ASSERT(level < MAX_EQ_BAND_STEPS);
#ifdef DSP_EQ_BAND_TUNING
    uint16_t reg_addr;
    uint8_t *p_reg_data = NULL;

    switch (band)
    {
        case EQ_BAND1:
            reg_addr = DSP_EQ_BAND1_REG_ADDR;
            p_reg_data = (uint8_t *)Adau1761_Band1;
            break;

        case EQ_BAND2:
            reg_addr = DSP_EQ_BAND2_REG_ADDR;
            p_reg_data = (uint8_t *)Adau1761_Band2;
            break;

        case EQ_BAND3:
            reg_addr = DSP_EQ_BAND3_REG_ADDR;
            p_reg_data = (uint8_t *)Adau1761_Band3;
            break;

        case EQ_BAND4:
            reg_addr = DSP_EQ_BAND4_REG_ADDR;
            p_reg_data = (uint8_t *)Adau1761_Band4;
            break;

        case EQ_BAND5:
            reg_addr = DSP_EQ_BAND5_REG_ADDR;
            p_reg_data = (uint8_t *)Adau1761_Band5;
            break;
        default:
            break;
    }
    ...
    if (p_reg_data)
    {
        p_reg_data += level * EQ_BAND_PARAM_TOTAL_SIZES_PER_STEP;
        Adau1761Drv_SafeLoadData(EQ_BAND_PARAM_LENGHT, p_reg_data, reg_addr);
    }
#endif
}
...
void Adau1761Drv_BypassEnable(bool enable)
{
    ...
    reg_addr_b = DSP_EQ_BYPASS_SWITCH_ADDR2;
    if( enable )
    {
...
bool Adau1761Drv_MusicDetected(void)
{
    ...
    Adau1761Drv_I2CRead(DSP_AUDIO_DETECTION_REG_ADDR, sizeof(data), data);
    if(data[1] == 0x80)
        return FALSE;
    else
        return TRUE;
...
void Adau1761Drv_WFMute(bool mute_enable)
{
    ...
    Adau1761Drv_I2CWrite(DSP_WF_MUTE_REG_ADDR_0, 4, (uint8_t *)(&reg_data));
    Adau1761Drv_I2CWrite(DSP_WF_MUTE_REG_ADDR_1, 4, (uint8_t *)(&reg_data));
}
...
void Adau1761Drv_TW1Mute(bool mute_enable)
{
    ...
    if(mute_enable)
        ...
        reg_data[1] = AUDIO_UNMUTE;
    Adau1761Drv_I2CWrite(DSP_TW_MUTE_REG_ADDR_0, 4, (uint8_t *)(&reg_data));
}
...
void Adau1761Drv_TW2Mute(bool mute_enable)
{
    ...
    if(mute_enable)
        ...
        reg_data[1] = AUDIO_UNMUTE;
    Adau1761Drv_I2CWrite(DSP_TW_MUTE_REG_ADDR_1, 4, (uint8_t *)(&reg_data));
}
...
{
    uint16_t reg_addr = DSP_SYS_MUTE_ADDR;
    uint8_t reg_data[4]= {0x00, 0x00, 0x00, 0x00};

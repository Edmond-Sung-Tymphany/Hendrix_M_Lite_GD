...
function build_target() 
{
	...
	if [ "" == "$piu_prebuilt_hex" ]; then
		...
		piu_tgt_hex=${model}_piu_swv${piu_version}_${tp_target}.hex
	...
	if [ "" == "$ubl_prebuilt_hex" ]; then
		...
		ubl_tgt_hex=${model}_ubl_swv${ubl_version}_${tp_target}.hex
	...
	if [ "release" == ${tp_target} ]; then
		...
		pwd
		cp "${sdf_file}" "${folder_target}/production_test/sdf_${model}_v${sw_ver}.json"
	fi
...
function build_start()
{
	...
	#UBL Version
	if [ "" == "$ubl_prebuilt_hex" ]; then
		uver0=$(sed -n 's/#define *BL_MAJOR_VERSION *\(\)/\1/p' model_config.h)
		uver1=$(sed -n 's/#define *BL_MINOR_VERSION1 *\(\)/\1/p' model_config.h)
		uver2=$(sed -n 's/#define *BL_MINOR_VERSION2 *\(\)/\1/p' model_config.h)
		uver3=$(sed -n 's/#define *BL_MINOR_VERSION3 *\(\)/\1/p' model_config.h)
	...
	#PIU Version
	if [ "" == "$piu_prebuilt_hex" ]; then
		pver0=$(sed -n 's/#define *PIU_MAJOR_VERSION *\(\)/\1/p' model_config.h)
		pver1=$(sed -n 's/#define *PIU_MINOR_VERSION1 *\(\)/\1/p' model_config.h)
		pver2=$(sed -n 's/#define *PIU_MINOR_VERSION2 *\(\)/\1/p' model_config.h)
		pver3=$(sed -n 's/#define *PIU_MINOR_VERSION3 *\(\)/\1/p' model_config.h)
	...
	if [ "1" != ${dsp_auto_build} ]; then
		mkdir "${out_folder}/BT"
		mkdir "${out_folder}/BT/OTA"
		cp -Rp bt_fw/* "${out_folder}/BT"
		cp -fp script/${model}/Joplin.xuv "${out_folder}/BT"
		cp -fp script/${model}/*.bin "${out_folder}/BT/OTA"
		md5sum ${out_folder}/BT/*.xuv >> ${out_folder}/md5sum.txt
		md5sum ${out_folder}/BT/OTA/*.bin >> ${out_folder}/md5sum.txt
	fi

	mkdir "${out_folder}/DOC"
	cp -fp conf/*.docx "${out_folder}/DOC"
	cp -fp conf/*.json "${out_folder}/DOC"
	cp -fp script/Burn_MCU_firmware.bat "${out_folder}/MCU"
	...
	if [ ${zip_en} == "1" ] && [ ${dsp_auto_build} != "1" ]; then
		cp "${tmp_folder}/"*.txt MCU/
		"${zip_path}" a "../Zound_${model}_hw${hw_ver}_swv${sw_version}_${date}.zip" $(ls -I $tmp_folder)
		# generate md5
		md5sum "../Zound_${model}_hw${hw_ver}_swv${sw_version}_${date}.zip" >> "../Zound_${model}_hw${hw_ver}_swv${sw_version}_${date}.zip.md5.txt"
	fi
	...
	if [ "1" == ${dsp_auto_build} ]; then
		...
		mkdir "${dsp_path}"
		cp ../DSP/${model}/ADAU*_1*.h "${dsp_path}"

...
//test
    ...
    AseFepSetAudioInputReq_AudioInput aseAudioInput = AseFepSetAudioInputReq_AudioInput_ASE;
...
QState MainApp_PoweringDown(cMainApp * const me, QEvt const * const e)
{
    switch (e->sig)
    {
        case Q_ENTRY_SIG:
        {
            ...
            MainApp_SendLedReq(me, LED_IND_ID_POWERING_DOWN);
            return Q_HANDLED();
        }
        case SYSTEM_MODE_RESP_SIG:
        {
...
QState MainApp_Active(cMainApp * const me, QEvt const * const e)
{
    ...
    switch (e->sig)
    {
        case Q_ENTRY_SIG:
        {
            ...
            AudioSrv_SetAudio(AUDIO_RESET_LINEIN_JACK_AND_MUSIC_STA_SETT_ID, /*No used*/TRUE, /*No used*/0, /*No used*/0);
            PersistantObj_RefreshTick((cPersistantObj*)me, MAINAPP_TIMEOUT_IN_MS);
        ...
        case AUDIO_MUSIC_STREAM_STATE_SIG:
        {
            ...
            if (pAudioMusicStateEvt->hasMusicStream)
            {
                if (pAudioMusicStateEvt->jackId == AUXIN_JACK)
                {
                    ...
                    AudioSrv_SetAudio(AUDIO_LINEIN_MULTI_ROOM_CHANNEL_SETT_ID, TRUE, me->sourceHandler[AUDIO_SOURCE_AUXIN].audioChannel, /*No used*/0);
                ...
                else if (pAudioMusicStateEvt->jackId == SPDIF1_IN_JACK)
                {
                    ...
                    AudioSrv_SetAudio(AUDIO_LINEIN_MULTI_ROOM_CHANNEL_SETT_ID, TRUE, me->sourceHandler[AUDIO_SOURCE_SPDIF_IN].audioChannel, /*No used*/0);
#ifdef SOURCE_HIJACK_IN_MCU
        ...
        case Q_EXIT_SIG:
        {
            ...
            QTimeEvt_disarm(TIME_EVT_OF(me));
            me->combinedKey = COM_KEY_INVALID;
            me->tickHandlers[TIMER_ID_VOLUME_CHANGE_TIMEOUT].timer = INVALID_VALUE;
            me->tickHandlers[TIMER_ID_DOUBLE_PRESS_TIMEOUT].timer = INVALID_VALUE;

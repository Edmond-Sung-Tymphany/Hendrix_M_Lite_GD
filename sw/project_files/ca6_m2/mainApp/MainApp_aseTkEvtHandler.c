...
static void MainApp_AseTkSetLineInSensitivity(cMainApp * const me, AseFepSetLineInSensitivity sens)
{
    if (sens.has_sensitivity)
    {
        ...
        AudioSrv_SetAudio(AUDIO_LINEIN_SENSITIVITY_SETT_ID, TRUE, (uint32)sens.sensitivity);
    }
}
...
static void MainApp_AseTkSetPositionSoundMode(cMainApp * const me, AseFepSetPositionSoundModeReq soundMode)
{
    ...
    if (soundMode.has_mode)
    {
        ...
        AudioSrv_SetAudio(AUDIO_POS_SOUND_MODE_SETT_ID, TRUE, NULL);
    }
}
...
static void MainApp_AseTkSetVolFade(cMainApp * const me, AseFepVolumeFadeEvent volFadeEvent)
{
    ...
    uint8 currentVol = *(uint8*)Setting_Get(SETID_VOLUME);
    if(volFadeEvent.start_volume != currentVol)
    {
        TP_PRINTF("    start_volume(%d) != currentVol(%d) \r\n", volFadeEvent.start_volume, currentVol);
        volFadeEvent.start_volume = currentVol;
    }
    uint32 fadeRange = TYM_DIFF(volFadeEvent.target_volume, volFadeEvent.start_volume);

    if(((fadeRange <= (MAX_VOLUME - MIN_VOLUME)) && fadeRange > 0)
        && (volFadeEvent.target_volume >= MIN_VOLUME && volFadeEvent.target_volume <= MAX_VOLUME)
        && (volFadeEvent.start_volume >= MIN_VOLUME && volFadeEvent.start_volume <= MAX_VOLUME))
    {
        tVolFadeParam volFadeParam = {0};
        volFadeParam.fadeType = (volFadeEvent.target_volume > volFadeEvent.start_volume)? TRUE : FALSE;
        volFadeParam.fadeStep = ADUIO_MIN_FADE_STEP;
        volFadeParam.fadeStepTime = (volFadeEvent.fade_duration * volFadeParam.fadeStep)/fadeRange;
        volFadeParam.targetVol = volFadeEvent.target_volume;
        if(ADUIO_MIN_FADE_STEP_TIME >= volFadeParam.fadeStepTime)
        {
            volFadeParam.fadeStepTime = ADUIO_MIN_FADE_STEP_TIME;
            volFadeParam.fadeStep = (volFadeParam.fadeStepTime * fadeRange)/volFadeEvent.fade_duration;
        }
        Setting_Set(SETID_VOLUME_FADE_PARAM, &volFadeParam);
        AudioSrv_SetAudio(AUDIO_VOL_FADE_SETT_ID, TRUE, NULL);
        if(me->isVolChanged == FALSE)
        {
            me->absoluteVol = volFadeParam.targetVol;
        }
    }
    else
    {
        TP_PRINTF("The volume fade parameters are invalid. \r\n");
        AseTkSrv_ReplyVolumeFade(currentVol);
    }
}
...
static QState MainApp_AseFepEventHandler(cMainApp * me, AseFepEvent aseFepEvent)
{
    ...
    switch (aseFepEvent.event)
    {
        ...
        {
            /* TODO: do not show on network standby mode, becuase when enter network standby mode, the event sequence is:
             *    NETWORK-STANDBY > PLAYER_STOPPED
             */
        ...
        case AseFepEvent_Event_SYSTEM_STATUS_STANDBY:
        {
            ...
            MainApp_SendLedReq(me, LED_IND_ID_NETWORK_STANDBY_MODE);
            MainApp_UpdateConnLed(me);
            if (me->isTonePlaying == FALSE)
            {
                AudioSrv_SetAudio(AMP_SLEEP_MODE_ID, TRUE, /* NO USED*/0);
            }
            break;
        }
        ...
        case AseFepEvent_Event_SYSTEM_STATUS_ON_NO_OPERATION:
        {
            ...
            MainApp_SendLedReq(me, LED_IND_ID_NETWORK_STANDBY_MODE); // product led go to dim.
...
QState MainApp_AseTkEvtHandler(cMainApp * const me, QEvt const * const e)
{
    ...
    switch(aseTkEvt->aseFepCmd.which_OneOf)
    {
        ...
        case AseFepMessage_aseFepVolumeFadeEvent_tag:
        {
            ...
            MainApp_AseTkSetVolFade(me, volFadeEvent);
            break;
        }

...
.coeff_miniDSP_D Anonymous_Block_1_Count =5
.codeblock Anonymous_Block_1 cycles=17, mincycle=256, maxcycle=272, target=miniDSP_D
acc_init Anonymous_Block_1_Count data_one ; Acc = N (start a delay loop)
nop
Anonymous_Block_1_NopLoop:
acc      MinusOne_Int data_one  ; Acc = N-1
jump     jmp_sz Anonymous_Block_1_NopLoop 7 ; stop when loop iterates n+2 times 
nop
.endcodeblock
.coeff_miniDSP_D MinusOne_Int =-1 $common
.coeff_miniDSP_D Anonymous_Block_2_Count =5
.codeblock Anonymous_Block_2 cycles=16, mincycle=474, maxcycle=489, target=miniDSP_D
acc_init Anonymous_Block_2_Count data_one ; Acc = N (start a delay loop)
nop
Anonymous_Block_2_NopLoop:
acc      MinusOne_Int data_one  ; Acc = N-1
jump     jmp_sz Anonymous_Block_2_NopLoop 7 ; stop when loop iterates n+2 times 
.endcodeblock
.coeff_miniDSP_D MinusOne_Int =-1 $common
.coeff_miniDSP_D Anonymous_Block_3_Count =121
.codeblock Anonymous_Block_3 cycles=248, mincycle=648, maxcycle=895, target=miniDSP_D
acc_init Anonymous_Block_3_Count data_one ; Acc = N (start a delay loop)
nop
Anonymous_Block_3_NopLoop:
acc      MinusOne_Int data_one  ; Acc = N-1
jump     jmp_sz Anonymous_Block_3_NopLoop 123 ; stop when loop iterates n+2 times 
.endcodeblock
